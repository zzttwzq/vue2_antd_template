    ...
    async getApiList(list) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            for (var j = 0; j < element.list.length; j++) {
                ...
                var filePath = configs.applicationRootPath + "json/data_" + element2._id + ".json";
                let data2 = [];
...
let fileCreatorUtils = {
    ...
    getMockObjectProperty: function(key, data, le) {
        ...
        } else if (data.type.indexOf('string') > -1 || data.type.indexOf('name') > -1) {
            params += `${le}'${key}': '@GOODS',\r\n`;
    ...
    getParamsDocText: function(obj) {
        ...
        if (request_params_string.indexOf(` ${obj.name} `) == -1) {
            if (obj.type == 'array') {
                request_params_string = ` * @param { ${obj.type ? obj.type : ''} } ${obj.name} - [ ${obj.items.type} ] ${req} ${obj.desc}\r\n`;
            } else if (obj.type == 'object') {
                request_params_string = ` * @param { ${obj.type ? obj.type : ''} } ${obj.name} - ${req} ${obj.desc}\r\n`;
            } else {
                request_params_string = ` * @param { ${obj.type ? obj.type : ''} } ${obj.name} - ${req} ${obj.desc}\r\n`;
            }
        }
...
let fileCreator = {
    ...
    createApis: async function(list) {
        ...
        list.forEach(element => {
            element.list.map(it => {
                ...
                let text = "    " + pathname.toUpperCase() + ": `${BASE_URL}/api" + path;
    ...
    createMockIndex: async function(list) {
        ...
        list.forEach(element => {
            ...
            let txt = `import '@/mock/${configs.nameMap[element.name] + '_mock'}'\r\n`;
...
`
    ...
    createYapiService: async function(data) {
        ...
        data.list.map(it => {
            ...
            /// query 请求参数
            it.req_query ? it.req_query.map(it2 => {
                request_params_string += fileCreatorUtils.getParamsDocText(it2);
            }) : []

            // body 请求参数
            it.req_body ? it.req_body.map(it2 => {
                request_params_string += fileCreatorUtils.getParamsDocText(it2);
            }) : []
            ...
            /// 使用api注释
            useText += `\r\n// ${it.method.toLowerCase() + '_' + pathname}(${request_params_string2}{});\r\n`;
 ...
 */
export async function ${it.method.toLowerCase() + '_' + pathname}(${request_params_string2 + 'params'}) {
    return request(${pathname.toUpperCase() + request_params_string3}, METHOD.${it.method.toUpperCase()}, params ? params : {}, null)
} \r\n`
        });
